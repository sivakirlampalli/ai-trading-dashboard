...
# In-memory dict for latest live signals per symbol
latest_signals = {}
...
API_KEY = "2LNVJFMX5J9F3UFJ"  # Insert your Alpha Vantage key here


def poll_and_update_signals():
    SYMBOLS = ["aapl", "goog", "msft", "bitcoin", "ethereum", "litecoin"]
    backoff_seconds = 5  # initial backoff delay in seconds
    max_backoff = 300    # maximum backoff delay (5 minutes)

    while True:
        for symbol in SYMBOLS:
            try:
                if symbol in ["AAPL", "GOOG", "MSFT"]:
                    ...
                    # Handle rate limit note from Alpha Vantage
                    if "Note" in data:
                        print(f"[Rate Limit] Backoff triggered for {symbol}: {data['Note']}")
                    ...
                    if not ts:
                        print(f"[No Data] Alpha Vantage returned no data for {symbol}")
                        latest_signals[symbol.upper()] = []
                        continue

                    df = pd.DataFrame([
                        {"timestamp": k, "close": float(v["4. close"])}
                        for k, v in ts.items()
                    ])
                    ...
                    for i in range(1, len(df)):
                        if pd.isna(df["sma_fast"].iloc[i-1]) or pd.isna(df["sma_slow"].iloc[i-1]):
                            continue
                        sma_fast = df["sma_fast"].iloc[i]
                        sma_slow = df["sma_slow"].iloc[i]
                        confidence = int(min(99, max(51, abs(sma_fast - sma_slow) / sma_slow * 100)))
                        if (sma_fast > sma_slow) and (df["sma_fast"].iloc[i-1] <= df["sma_slow"].iloc[i-1]):
                            signals.append({
                                "type": "buy",
                                "confidence": confidence,
                                "timestamp": df["timestamp"].iloc[i]
                            })
                        elif (sma_fast < sma_slow) and (df["sma_fast"].iloc[i-1] >= df["sma_slow"].iloc[i-1]):
                            signals.append({
                                "type": "sell",
                                "confidence": confidence,
                                "timestamp": df["timestamp"].iloc[i]
                            })
                    latest_signals[symbol.upper()] = signals
                else:
                    ...
                    if not data:
                        print(f"[No Data] CoinGecko returned no data for {symbol}")
                        latest_signals[symbol.upper()] = []
                        continue
        ...
        time.sleep(60)  # Base loop delay - adjust as needed


threading.Thread(target=poll_and_update_signals, daemon=True).start()

@app.get("/api/latest-signals")
def get_latest_signals(symbol: str):
    # Defensive: always returns a list
    return latest_signals.get(symbol.upper(), [])

# (Other endpoints remain as in your file)
# ...
# Make sure your frontend uses symbols exactly as listed above ("AAPL", "bitcoin", etc.)

# Leave all your existing authentication, upload, CSV, and database logic the same.
...
API_KEY = "HPVPXWQQXNEA6D3M" # Replace this with your actual key 
...
def stock_signals_sma(symbol: str = Query(...)): 
    ...
    for i in range(1, len(df)): 
        ...
        if (df["sma_short"].iloc[i] > df["sma_long"].iloc[i]) and (df["sma_short"].iloc[i-1] <= df["sma_long"].iloc[i-1]): 
            ...
            confidence = int(
                min(99, max(51, abs(sma_fast - sma_slow) / sma_slow * 100))
            )  # Ensure the confidence is between 51% and 99%

            signals.append({"type": "buy", "confidence": confidence, "timestamp": df["date"].iloc[i]}) 
            ...
            signals.append({"type": "sell", "confidence": confidence, "timestamp": df["date"].iloc[i]}) 
    return signals 
...
def live_signals_sma(symbol: str): 
    ...
    for i in range(1, len(df)): 
        ...
        # Crossover logic 
        if (df["sma_short"].iloc[i] > df["sma_long"].iloc[i]) and (df["sma_short"].iloc[i-1] <= df["sma_long"].iloc[i-1]):
            # Calculate the confidence as the relative difference between moving averages
            sma_fast = df["sma_fast"].iloc[i]
            sma_slow = df["sma_slow"].iloc[i]
...
def fetch_crypto_ohlc(symbol: str = Query(...), days: int = 7): 
    ...
    response = requests.get(url, params=params)
    data = response.json()
    if not isinstance(data, list):   # If not OHLC array
        print(f"CoinGecko OHLC error: {data}")
        # Optionally, return an empty array or cached data instead!
        return []
    return data
...
def live_signals(symbol: str): 
    ...
    for i in range(1, len(df)): 
        if df["sma_fast"].iloc[i] > df["sma_slow"].iloc[i] and df["sma_fast"].iloc[i-1] <= df["sma_slow"].iloc[i-1]:
            ...
            signals.append({"type": "buy", "confidence": confidence, "timestamp": df.index[i]}) 
            ...
            signals.append({"type": "sell", "confidence": confidence, "timestamp": df.index[i]}) 
